...
export default function LoginPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        catch (err) { setError(err.response?.data?.message || "Nepareizs e-pasts vai parole"); }
    ...
    return (
        <div className="min-h-[70vh] flex items-center justify-center px-4">
            <div className="w-full max-w-md">
                <h1 className="text-3xl font-bold text-content-primary text-center mb-8">Pieslēgties</h1>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        <div>
                            ...
                            <input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} required
                                className="w-full bg-surface-tertiary border border-border text-content-primary rounded-lg px-4 py-3 focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent" placeholder="tavs@epasts.lv" />
                        </div>
                        <div>
                            <label className="block text-content-secondary text-sm mb-1.5">Parole</label>
                            <input type="password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} required
                                className="w-full bg-surface-tertiary border border-border text-content-primary rounded-lg px-4 py-3 focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent" placeholder="••••••••" />
                        </div>
