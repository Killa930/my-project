/*
 * SellPage — страница "Pārdot auto"
 *
 * Форма с данными автомобиля. Доступна всем (и гостям).
 * Но при нажатии "Izveidot piedāvājumu":
 * - Если залогинен → создаёт объявление (как CreateCarPage)
 * - Если НЕ залогинен → показывает модальное окно "Izveidot kontu"
...
export default function SellPage() {
    ...
    useEffect(() => {
        ...
        // Если залогинен — предзаполняем контактные данные
        if (user) {
            setForm((prev) => ({
                ...prev,
    ...
    const handleChange = (k, v) => {
        setForm((p) => {
            const u = { ...p, [k]: v };
            if (k === "manufacturer_id") u.car_model_id = "";
            return u;
        });
    };
    ...
    const handleImageChange = (e) => {
        ...
        files.forEach((f) => {
            ...
            r.onloadend = () => {
                setImagePreviews((p) => [...p, { file: f, url: r.result }]);
            };
            r.readAsDataURL(f);
        });
    };
    ...
    const handleSubmit = async (e) => {
        ...
        // Если не залогинен — показываем модальное окно
        if (!user) {
    ...
    const ic = (f) =>
        `w-full bg-surface-tertiary border ${errors[f] ? "border-status-danger" : "border-border"} text-content-primary rounded-lg px-4 py-3 focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent`;
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
            ...
            <div className="text-center mb-10">
                ...
                <p className="text-content-secondary mt-2">
                    Aizpildiet formu un jūsu sludinājums tiks publicēts
                    platformā
                </p>
            </div>
            ...
            <form
                ...
                className="bg-surface-secondary border border-border rounded-xl p-6 sm:p-8 space-y-8"
            >
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-3 sm:grid-cols-5 gap-3 mb-3">
                            {imagePreviews.map((p, i) => (
                                <div
                                    key={i}
                                    className="relative aspect-square bg-surface-tertiary rounded-lg overflow-hidden"
                                >
                                    <img
                                        src={p.url}
                                    ...
                                    <button
                                        ...
                                        className="absolute top-1 right-1 bg-status-danger text-white rounded-full w-5 h-5 flex items-center justify-center"
                                    >
                                        <XMarkIcon className="w-3 h-3" />
                                    </button>
                                    {i === 0 && (
                                        <span className="absolute bottom-1 left-1 bg-accent text-content-inverted text-[10px] font-bold px-1.5 py-0.5 rounded">
                                            Galvenais
                                        </span>
                                    )}
                                </div>
                            ...
                            <label className="aspect-square bg-surface-tertiary border-2 border-dashed border-border hover:border-accent rounded-lg flex flex-col items-center justify-center cursor-pointer transition-colors">
                                <PhotoIcon className="w-8 h-8 text-content-muted" />
                                <span className="text-content-muted text-xs mt-1">
                                    Pievienot
                                </span>
                                <input
                                    type="file"
            ...
            {showAuthModal && (
                ...
                >
                    <div
                        className="bg-surface-secondary border border-border rounded-2xl p-8 max-w-md w-full"
                    ...
                    >
                        ...
                        <div className="text-center mb-6">
                            ...
                            <p className="text-content-secondary text-sm mt-2">
                                Lai publicētu sludinājumu, nepieciešams
                                reģistrēties vai pieslēgties.
                            </p>
                        </div>
                        ...
                        <div className="space-y-3">
                            <Link
                                to="/register"
                                className="block w-full bg-accent hover:bg-accent-hover text-content-inverted text-center py-3 rounded-lg font-semibold transition-colors"
                            >
                                Reģistrēties
                            </Link>
                            <Link
                                to="/login"
                                className="block w-full bg-surface-tertiary hover:bg-border-hover text-content-primary text-center py-3 rounded-lg font-semibold transition-colors"
                            >
                                Pieslēgties
                            </Link>
                        </div>
                        ...
                        <button
                            onClick={() => setShowAuthModal(false)}
                            className="block w-full text-content-muted text-sm mt-4 hover:text-content-secondary transition-colors"
                        >
                            Atcelt
                        </button>
                    </div>
